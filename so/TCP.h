...
Ku_short Kntohs(Ku_short Knetshort);
 ...
 Kin_addr Kinet_addr(const char* Kcp, int af=AF_INET);	// windows

 const char* Kinet_ntop(const char* Kin,int af=AF_INET);
 const char* Kinet_ntop(const unsigned int* Kin, int af = AF_INET);
 const char* Kinet_ntop(const unsigned long* Kin, int af = AF_INET);
 enum Ksocketenum {
	KSOCKET_SUC,
	KSOCKET_ERR
};
